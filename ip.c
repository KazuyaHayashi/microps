...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (!iface) {
        errorf("interface not found, dev=%s", dev->name);
        return;
    }
...
static ssize_t
ip_output_core(struct ip_iface *iface, uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t dst, uint16_t id, uint16_t offset)
{
